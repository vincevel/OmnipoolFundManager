...
<a href="#" class="d-block">{{ ucwords($user1->first_name) }} {{ ucwords($user1->last_name) }}</a>
</div>
</div>
...
<li class="nav-item menu-open" style="width: 100%;border: 0px solid red" >
...
<div class="card">
	<div class="card-header">
		<h2 class="pt-2  ">{{ ucwords($user1->last_name) }}, {{ ucwords($user1->first_name) }}</h2>   <h5>ID# {{ $user1->id - 2 }}</h5>
	</div>
...
<strong>Copyright &copy; {{ $current_year }} <a href="https://omnipool.co">{{ config('app.name', 'OmniPool') }}</a>.</strong> All rights reserved.
</footer>
</div>
